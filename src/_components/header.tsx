...
import { Command, NotepadText, Notebook, Projector, Menu } from 'lucide-react'
                  ...
                  <ul className="flex flex-col items-start">
                    {navLinks.map(({ icon: Icon, href, content }) => (
                      <li key={href} className="w-full ">
                        <Link
                          className={cn(
                            'flex py-2 rounded hover:bg-accent/40 w-full gap-1 items-center justify-center text-sm text-muted hover:text-foreground transition-colors font-medium',
          ...
          <ul className="flex gap-3 items-center">
            {navLinks.map(({ icon: Icon, href, content }) => (
              <li key={href}>
                <Link
                  className={cn(
                    'flex gap-1 items-center text-sm text-muted hover:text-foreground transition-colors font-medium',
                    pathname === href ? 'text-highlight' : '',
                  )}
                  href={href}
                >
